...
class el_t
{
 private:  // these will change depending
           // on the client needs
  int key;      // key
  string name;  // employee age
  int age; // employee age
  string gender;  //employee gender
  int bank; // last 4 of banking info for key
  bool employed; //displays if the employee is still employed

 public:

  el_t();  // to create a blank el_t object
  el_t(int, string, int, string, bool); // to create an initialized el_t object - useful for the client
 ...
 // use different el_t without modifying the code
  ...
  //friend ostream& operator<<(ofstream&, const el_t&);
